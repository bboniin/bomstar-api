...
class UserTransferPointsService {
    async execute({ user_receive_id, user_id, value }: TransferRequest) {
        
        if (!user_receive_id || !value) {
            throw new Error("Id do Usuário que vai enviar e valor são obrigatórios")
        }
...
export { UserTransferPointsService }